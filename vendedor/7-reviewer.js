...
1. Contem @handle no corpo do texto? (ex: @usuario ou nome proprio)
2. Abre com frase batida? ("Vi seu perfil", "Parabens pelo conteudo", "Notei que", "Acompanho seu")
3. Tem menos de 3 linhas sem estrutura completa (hook + valor + pergunta)?
4. Menciona preco, valor ou plano na primeira mensagem?
5. Tom de vendedor formal? ("Estou oferecendo", "Aproveite", "Oportunidade unica", "Solucao", "Servicos")
6. E generica demais? (poderia ser enviada para QUALQUER pessoa sem mudanca)
7. Nao termina com pergunta de resposta simples (sim/nao ou 1 palavra)?
...
- Score 60-79: aprovada com sugestao de melhoria opcional
...
Retorne JSON:
{
  ...
  "problemas": ["lista de problemas especificos encontrados, ou vazio se nenhum"],
  "pontos_positivos": ["lista do que a mensagem fez certo"],
  "versao_melhorada": "mensagem reescrita se score < 80, ou null se aprovada",
  "justificativa_melhoria": "o que foi mudado e por que, ou null se aprovada",
  "resumo_revisor": "frase curta resumindo a avaliacao"
}

Se precisar reescrever, siga:
- Estrutura: [HOOK especifico] + [VALOR concreto] + [PERGUNTA simples]
- Min 3 linhas, max 4 linhas
- NUNCA @handle no texto
- Tom: colega util, nao vendedor
- Use o gancho dos posts se disponivel
...
RESPONDA APENAS O JSON.`;
  ...
  try {
    ...
    if (review.problemas?.length) {
      console.log(`\n${C.yellow}  PROBLEMAS ENCONTRADOS:${C.reset}`);
      review.problemas.forEach(p => console.log(`  \u26a0\ufe0f  ${p}`));
    }
    ...
    if (review.versao_melhorada) {
      ...
      console.log(`\n${C.bright}  ${C.green}MENSAGEM FINAL (USE ESTA)${C.reset}${C.bright}:${C.reset}`);
    } else {
