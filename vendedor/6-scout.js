...
const NICHOS = {
  ...
  'lp-infoprodutor': {
    ...
    hashtags: ['#infoprodutor', '#cursoonline', '#mentoria', '#lançamento', '#coachdigital', '#infoproduto'],
...
async function gerarGuiaProspeccao(nicho = 'api-automacao', quantidade = 10) {
  ...
  console.log(`${C.bright}  SCOUT AI — GUIA DE PROSPECÇÃO DO DIA${C.reset}`);
...
{"perfis": [{"username": "", "bio": "", "seguidores": 0, "posts": 0, "por_que_bom_lead": ""}]}`;
  ...
  try {
    ...
    console.log(`\n${C.yellow}━━━ EXEMPLOS DE PERFIS-ALVO (use como referência no Instagram) ━━━${C.reset}`);
    console.log(`${C.cyan}  Esses são perfis fictícios. Busque perfis SIMILARES no Instagram.${C.reset}`);
    ...
    console.log(`  ${C.green}  node 5-orchestrator.js analyze @usuario "bio" seguidores posts${C.reset}`);
    console.log(`  3. Copie a mensagem gerada e envie no DM`);
    console.log(`  4. Depois rode: node 5-orchestrator.js sent @usuario`);
    console.log(`\n  Guia salvo em: ${arquivoGuia}`);
    console.log(`${C.magenta}${'='.repeat(60)}${C.reset}\n`);

  } catch (error) {
    console.error('[SCOUT] Erro ao gerar exemplos:', error.message);
    console.log('[SCOUT] Guia de hashtags e keywords acima ainda é válido — use para prospectar!');
  }
}
