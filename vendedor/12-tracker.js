...
function getMensagemFinal(data) {
  if (data.revisao?.mensagem_final) return data.revisao.mensagem_final.slice(0, 80);
  const recKey = `mensagem_${data.mensagens?.mensagem_recomendada}`;
  return (data.mensagens?.[recKey]?.texto || '').slice(0, 80);
}
...
function cmdList() {
  ...
  if (semRevisao.length) {
    ...
    for (const m of semRevisao) {
      console.log(`  ${C.dim}@${m.username}${C.reset}`);
    }
  }
...
function cmdUpdate(username, outcome, extra) {
  ...
  if (isFirst) {
    ...
    // Atualizar resultado
    if (!data.tracking.data_envio) {
      // Auto-marcar envio se ainda nao foi marcado
      data.tracking.data_envio = now;
    }
    ...
    if (outcome === 'respondeu' || outcome === 'negociando') {
  ...
  const angle = data.mensagens?.[`mensagem_${data.mensagens?.mensagem_recomendada}`]?.angulo || '';
...
function cmdStats() {
  ...
  if (receita > 0) console.log(`  Receita total:     ${C.green}${C.bright}R$ ${receita.toLocaleString('pt-BR', {minimumFractionDigits: 2})}${C.reset}`);

  if (scoreResponderam || scoreIgnoraram) {
    console.log(`\n${C.bright}  SCORE vs OUTCOME (correlacao)${C.reset}`);
    console.log(`${'\u2500'.repeat(50)}`);
    if (scoreConverteram) console.log(`  Score medio converteram: ${C.green}${C.bright}${scoreConverteram}/100${C.reset}`);
    if (scoreResponderam) console.log(`  Score medio responderam: ${C.cyan}${scoreResponderam}/100${C.reset}`);
    if (scoreIgnoraram)   console.log(`  Score medio ignoraram:   ${C.dim}${scoreIgnoraram}/100${C.reset}`);
    if (scoreResponderam && scoreIgnoraram) {
      const diff = scoreResponderam - scoreIgnoraram;
      const diffColor = diff > 0 ? C.green : C.red;
      console.log(`  Diferenca: ${diffColor}${diff > 0 ? '+' : ''}${diff} pts${C.reset} ${diff > 5 ? '(score do reviewer correlaciona com resposta \u2714\ufe0f)' : '(correlacao fraca \u26a0\ufe0f)'}`);
    }
  }
  ...
  console.log(`\n${C.dim}  Ultima atualizacao: ${new Date().toLocaleString('pt-BR')}${C.reset}\n`);
}
...
switch (cmd) {
  ...
  case 'sent':    if (!username) { console.error(`${C.red}Informe o username${C.reset}`); process.exit(1); } cmdUpdate(username, 'enviada', extra); break;
  case 'respondeu':
  case 'ignorou':
  case 'negociando':
  case 'converteu':
